...
class NotifyPromptTests(unittest.TestCase):
    ...
    def test_render_confirmation_prompt_tells_agent_to_wait_without_tools_or_file_reads(self):
        ...
        self.assertIn("Ignore this update", prompt)
        self.assertIn("PR Watch: Is this the right session for PR sendbird/ai-agent-js#1049?", prompt)
        self.assertIn("sendbird/ai-agent-js#1049", prompt)
        self.assertNotIn("\n\nsendbird/ai-agent-js#1049\n", prompt)
        self.assertIn("bang9 pushed new commits to PR #1049", prompt)
        self.assertNotIn("requested attention", prompt)
        self.assertNotIn("Link:", prompt)
        self.assertNotIn("Confirmation event id", prompt)
        self.assertNotIn("Candidate session", prompt)
        self.assertNotIn("Confirm command", prompt)
        self.assertNotIn("pr-watch:", prompt)
